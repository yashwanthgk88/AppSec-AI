...
controls_data = [
    ("Web Application Firewall (WAF)", "AWS WAF with OWASP CRS v3.3 ruleset protecting all public-facing APIs and web application. Includes custom rules for banking-specific attack patterns.", "preventive", "implemented", ["spoofing", "tampering", "information_disclosure"], 0.85, "Platform Security", "WAF dashboard: https://console.aws.amazon.com/wafv2/"),
    ("Multi-Factor Authentication (MFA)", "Adaptive MFA using FIDO2/WebAuthn, TOTP, and SMS OTP for customer authentication. Risk-based step-up for elevated risk scores.", "preventive", "implemented", ["spoofing"], 0.92, "Identity & Access", "MFA policy document v2.1"),
    ("Data Encryption at Rest (AES-256)", "AES-256-GCM encryption for all PII/PCI data in PostgreSQL using AWS RDS encryption and application-level column encryption for SSN/account numbers.", "preventive", "implemented", ["information_disclosure"], 0.95, "Data Security", "Encryption audit report Q1 2026"),
    ("TLS 1.3 in Transit Encryption", "TLS 1.3 enforced on all external connections. TLS 1.2 minimum for internal service-to-service communication. HSTS enabled with preload.", "preventive", "partial", ["information_disclosure", "tampering"], 0.80, "Infrastructure Security", "TLS configuration audit — TLS 1.0/1.1 still enabled on legacy endpoints"),
    ("Real-Time Fraud Detection Engine", "ML-based transaction monitoring consuming Kafka events. Rule-based velocity checks, geo-anomaly detection, and behavioral biometrics scoring.", "detective", "implemented", ["spoofing", "tampering", "elevation_of_privilege"], 0.88, "Fraud Operations", "Fraud engine dashboard: https://fraud-ops.internal/"),
    ("Centralized Audit Logging (SIEM)", "All application events forwarded to Splunk SIEM with 13-month retention. Alerts on auth failures, privilege changes, and data access anomalies.", "detective", "partial", ["repudiation"], 0.65, "Security Operations", "Splunk dashboards — admin action logging incomplete per SOX audit finding"),
    ("OFAC/SDN Sanctions Screening", "Real-time screening of all wire transfer beneficiaries against OFAC SDN list, EU sanctions, and UK HMT lists. Includes fuzzy name matching.", "preventive", "implemented", ["elevation_of_privilege"], 0.90, "BSA/AML Compliance", "Sanctions screening vendor: Dow Jones Risk & Compliance"),
    ("Role-Based Access Control (RBAC)", "Four-tier RBAC hierarchy (teller, supervisor, compliance, admin) enforced at API gateway and application layer. Least-privilege principle.", "preventive", "implemented", ["elevation_of_privilege", "spoofing"], 0.82, "Identity & Access", "RBAC matrix document v3.0"),
    ("API Rate Limiting", "API gateway rate limiting: 100 req/min per authenticated user, 20 req/min for unauthenticated endpoints. Progressive delays on auth endpoints.", "preventive", "planned", ["denial_of_service", "spoofing"], 0.50, "API Platform", "Rate limiting design doc — implementation scheduled for Sprint 24"),
    ("Container Image Scanning", "Trivy scanning on all container images in CI/CD pipeline. Blocks deployment of images with critical/high CVEs. Weekly re-scan of running images.", "detective", "implemented", ["tampering"], 0.78, "DevSecOps", "CI/CD pipeline: https://github.com/apex-financial/apex-banking-platform/actions"),
    ("DDoS Protection (AWS Shield)", "AWS Shield Advanced with CloudFront for DDoS mitigation. Auto-scaling configured for burst traffic handling.", "preventive", "implemented", ["denial_of_service"], 0.90, "Infrastructure Security", "AWS Shield subscription active"),
    ("Secrets Management (AWS Secrets Manager)", "Centralized secrets management for API keys, database credentials, and encryption keys. Automatic rotation for database passwords every 30 days.", "preventive", "partial", ["information_disclosure"], 0.60, "Platform Security", "Migration in progress — 8 of 15 secrets migrated from config files"),
    ("Input Validation Framework", "Custom validation framework using Bean Validation (JSR 380) with banking-specific validators for account numbers, routing numbers, and monetary amounts.", "preventive", "implemented", ["tampering", "information_disclosure"], 0.75, "Application Security", "Validation framework docs in Confluence"),
    ("Vulnerability Management Program", "Monthly vulnerability scans (Qualys), annual penetration test (NCC Group), and continuous SAST/SCA in CI/CD pipeline.", "detective", "implemented", ["tampering", "information_disclosure", "elevation_of_privilege"], 0.80, "Application Security", "Latest pentest report: NCC Group Feb 2026"),
    ("Incident Response Plan", "Documented IR plan with 4-hour SLA for critical security incidents. Tabletop exercises conducted quarterly. Integration with PagerDuty for on-call rotation.", "corrective", "implemented", ["spoofing", "tampering", "repudiation", "information_disclosure", "denial_of_service", "elevation_of_privilege"], 0.70, "Security Operations", "IR playbook v4.2 in Confluence"),
]

for ctrl in controls_data:
    name, desc, ctype, status, stride, eff, owner, evidence = ctrl
    c.execute("""
        INSERT INTO security_controls (project_id, name, description, control_type, status, stride_categories, effectiveness, owner, evidence, created_by, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (PROJECT_ID, name, desc, ctype, status, json.dumps(stride), eff, owner, evidence, USER_ID, NOW))
