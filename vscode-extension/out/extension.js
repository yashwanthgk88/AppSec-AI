...
async function activate(context) {
    try {
        ...
        context.subscriptions.push(vscode.commands.registerCommand('appsec.deepScan', async () => {
 ...
 */
async function deepScanCommand(context) {
    if (!await apiClient.isAuthenticated()) {
        const login = await vscode.window.showWarningMessage('Please login to SecureDev AI platform first', 'Login');
        if (login === 'Login') {
            await vscode.commands.executeCommand('appsec.login');
    ...
    try {
        updateStatusBar('scanning');
        const results = await scanProgressManager.showScanProgress(`Deep Scanning ${fileName}`, async (updateProgress) => {
            updateProgress({
                stage: 'initializing',
            ...
            if (scanResults.interprocedural?.taint_findings) {
                scanResults.interprocedural.taint_findings.forEach((finding) => {
                    deepFindings.push({
                        ...finding,
            ...
            const otherFindings = existingFindings.filter((f) => (f.file || f.location?.file) !== fileUri.fsPath);
            ...
            if (deepFindings.length > 0) {
                enhancedFindings = deepFindings.map(f => ({
                    ...
                    taintFlow: f.taintFlow || (f.call_chain ? {
                        source: { id: 'src', name: f.call_chain[0] || 'source', category: 'user-input', pattern: { type: 'call' }, description: 'Taint source' },
                        sink: { id: 'sink', name: f.call_chain[f.call_chain.length - 1] || 'sink', category: 'dangerous-call', pattern: { type: 'call' }, vulnerabilityType: f.type, description: 'Taint sink' },
                        taintedValue: { variable: 'data', source: null, location: f.location, path: [] },
                        path: f.call_chain.map((func, idx) => ({
                            ...
                            node: { type: 'Call', location: f.location }
                        })),
                        sanitizers: []
                    } : undefined)
                }));
        ...
        if (totalFindings > 0) {
            const interproceduralCount = results.deepFindings?.filter((f) => f.call_chain?.length > 0).length || 0;
            const action = await vscode.window.showWarningMessage(`🔗 Deep scan found ${totalFindings} issue(s) (${interproceduralCount} cross-function flows) in ${fileName}`, 'View Taint Flows', 'View Details');
            ...
            vscode.window.showInformationMessage(`✅ Deep scan complete: No cross-function vulnerabilities found. Analyzed ${functionsAnalyzed} functions.`);
        }
    }
    catch (error) {
        updateStatusBar('error');
