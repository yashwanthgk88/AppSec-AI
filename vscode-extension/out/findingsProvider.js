...
class FindingsProvider {
    ...
    async getChildren(element) {
        ...
        if (!element) {
            ...
            if (!isAuthenticated) {
                const loginItem = new FindingItem('🔐 Login to view findings', vscode.TreeItemCollapsibleState.None, 'login-prompt', undefined);
                loginItem.command = {
            ...
            if (totalFindings === 0) {
                const scanItem = new FindingItem('🔍 Run scan to detect vulnerabilities', vscode.TreeItemCollapsibleState.None, 'scan-prompt', undefined);
                scanItem.command = {
