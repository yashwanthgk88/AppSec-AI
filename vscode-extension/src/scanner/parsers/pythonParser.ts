...
export class PythonParser extends BaseParser {
    ...
    private parseImport(line: string, lineNum: number): ImportNode | null {
        const trimmed = line.trim();
    ...
    private parseClass(lines: string[], startLine: number): { node: ClassNode | null; endLine: number } {
        ...
        const members: (MethodNode | PropertyNode)[] = [];
    ...
    private parseFunction(lines: string[], startLine: number): { node: FunctionNode | null; endLine: number } {
        const line = lines[startLine];
        const trimmed = line.trim();
    ...
    private parseMethod(lines: string[], startLine: number, className: string): { node: MethodNode | null; endLine: number } {
        const line = lines[startLine];
        const trimmed = line.trim();
    ...
    private parseParameters(paramsStr: string, lineNum: number): ParameterNode[] {
    ...
    private parseBlock(lines: string[], startLine: number, baseIndent: number): { body: BlockNode; endLine: number } {
    ...
    private parseIf(lines: string[], startLine: number): { node: IfNode | null; endLine: number } {
        const line = lines[startLine];
        const trimmed = line.trim();
    ...
    private parseFor(lines: string[], startLine: number): { node: ForEachNode | null; endLine: number } {
        const line = lines[startLine];
        const trimmed = line.trim();
    ...
    private parseWhile(lines: string[], startLine: number): { node: IRNode | null; endLine: number } {
        const line = lines[startLine];
        const trimmed = line.trim();
    ...
    private parseTry(lines: string[], startLine: number): { node: TryNode | null; endLine: number } {
        const line = lines[startLine];
        const baseIndent = this.getIndent(line);
        ...
        let finalizer: BlockNode | undefined;
        let endLine = tryEnd;
        ...
        while (lineNum < lines.length) {
            ...
            if (trimmed.startsWith('except')) {
                ...
                if (exceptMatch) {
                    ...
                    const param: ParameterNode | undefined = exceptMatch[2] ? {
                        type: 'Parameter',
                        name: exceptMatch[2],
    ...
    private parseReturn(line: string, lineNum: number): ReturnNode {
        const trimmed = line.trim();
    ...
    private parseAssignment(line: string, lineNum: number): VariableDeclarationNode | AssignmentNode | null {
        const trimmed = line.trim();
    ...
    private parseClassProperty(line: string, lineNum: number): PropertyNode | null {
        const trimmed = line.trim();
        const assignMatch = trimmed.match(/^(\w+)(?:\s*:\s*([\w\[\],\s]+))?\s*=\s*(.+)$/);
    ...
    private parseExpressionStatement(line: string, lineNum: number): IRNode | null {
        const trimmed = line.trim();
    ...
    private parseExpression(expr: string, lineNum: number): IRNode | null {
    ...
    private parseCallExpression(expr: string, lineNum: number): CallExpressionNode | null {
    ...
    private hasDecorator(lines: string[], funcLine: number, decorator: string): boolean {
